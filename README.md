...
## Get Started
1. **Initial Machine Setup**. First time running the starter kit? Then complete the [Initial Machine Setup](https://github.com/tinkerware/provision#initial-machine-setup).
2. **Clone the project**. `git clone https://github.com/tinkerware/provision.git`.  
 ...
 * Run this to [increase the limit](http://stackoverflow.com/questions/16748737/grunt-watch-error-waiting-fatal-error-watch-enospc) on the number of files Linux will watch. [Here's why](https://github.com/tinkerware/provision/issues/6).    
...
*Note:* When you run `npm start`, no JS is minified. Why? Because minifying slows the build. So JS is only minified when you run the `npm run build` script. See [more on building for production below](https://github.com/tinkerware/provision#how-do-i-deploy-this).
